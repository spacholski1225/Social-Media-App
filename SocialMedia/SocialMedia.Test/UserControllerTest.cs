...
namespace SocialMedia.Test
{
    public class UserControllerTest
    {
        ...
        {
            //Arrange
            _mockUserRepository.Setup(s => s.GetUserByUserNameAsync(It.IsAny<string>())).ReturnsAsync(() => null);
            //Act
        ...
        {
            //Arrange
            _mockUserRepository.Setup(s => s.GetUserByUserNameAsync(It.IsAny<string>())).ReturnsAsync(new IdentityUser());
            //Act
            var result = await _controller.GetUserByUserName("test");
            //Assert
            Assert.IsType<OkObjectResult>(result);
